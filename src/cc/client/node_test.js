define(function(require, exports, module) {
  ...
  describe("node.js", function() {
    ...
    describe("group", function() {
      it("create", function() {
        var g0 = cc.createGroup(cc.client.rootNode, 1);
        assert.isTrue(cc.instanceOfNode(g0));
        assert.isTrue(cc.instanceOfGroup(g0));
        assert.deepEqual(tl, ["/g_new", g0.nodeId, 1, cc.client.rootNode.nodeId]);
      });
    });
    describe("synth", function() {
      ...
      it("create", function() {
        var s0 = cc.createSynth(cc.client.rootNode, 1, def, {f:100});
        assert.isTrue(cc.instanceOfNode(s0));
        assert.isTrue(cc.instanceOfSynth(s0));
        assert.deepEqual(tl, ["/s_new", s0.nodeId, 1, cc.client.rootNode.nodeId, def._defId, [0, 100]]);
      });
      it("#set", function() {
    ...
    describe("interface", function() {
      ...
      it("Group.after", function() {
        ...
        cc.instanceOfGroup(g);
        assert.deepEqual(tl, ["/g_new", g.nodeId, C.ADD_AFTER, cc.client.rootNode.nodeId]);
      });
      it("Group.before", function() {
        var g = cc.global.Group.before();
        cc.instanceOfGroup(g);
        assert.deepEqual(tl, ["/g_new", g.nodeId, C.ADD_BEFORE, cc.client.rootNode.nodeId]);
      });
      it("Group.head", function() {
        var g = cc.global.Group.head();
        cc.instanceOfGroup(g);
        assert.deepEqual(tl, ["/g_new", g.nodeId, C.ADD_TO_HEAD, cc.client.rootNode.nodeId]);
      });
      it("Group.tail", function() {
        var g = cc.global.Group.tail();
        cc.instanceOfGroup(g);
        assert.deepEqual(tl, ["/g_new", g.nodeId, C.ADD_TO_TAIL, cc.client.rootNode.nodeId]);
      });
    });
